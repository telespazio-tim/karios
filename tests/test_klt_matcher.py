...
import pytest
from karios.matcher.klt import KLT, klt_tracker, __filter_outliers
...
def test_klt_tracker_with_features(mock_cv2):
    ...
    mock_cv2.calcOpticalFlowPyrLK.return_value = (mock_points, np.array([1, 1, 1]), np.array([[0.1], [0.2], [0.15]]))
...
def test_klt_tracker_outliers_filtering(mock_cv2):
    ...
    # Mock OpenCV functions
    mock_points = np.array([[[10, 20]], [[15, 25]], [[30, 40]], [[90, 90]]], dtype=np.float32)  # 90,90 is an outlier
    mock_cv2.goodFeaturesToTrack.return_value = mock_points
